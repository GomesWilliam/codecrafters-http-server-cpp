...
Request::ParseRequest(const std::string &request) {
    ...
    while (current_pos < request.length()){
        ...
        std::string header_line = request.substr(current_pos, line_end - current_pos);
...
Request::IsValidMethod(const std::string &method){
    return std::find(valid_methods_.begin(), valid_methods_.end(), method) != valid_methods_.end();
}
